...
do_backup() {
  ...
  if [[ $status != 0 ]]; then
    echo "BACKUP_ERROR: Pre-backup script exited with status code $status. Not performing backup." >&2
    return
  fi
  ...
  status=$?
  if [[ $status != 0 ]]; then
    echo "BACKUP_ERROR: duplicacy exited with status code $status." >&2
    return
  fi
...
export DUPLICACY_B2_ID=`cat $B2_ID_FILE 2>/dev/null`
export DUPLICACY_B2_KEY=`cat $B2_KEY_FILE 2>/dev/null`
